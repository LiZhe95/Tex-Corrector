...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.convs = nn.ModuleList([nn.Conv1d(in_channels=hid_dim,
                                    ...
                                    for _ in range(n_layers)])
        self.dropout = nn.Dropout(dropout)
        self.demo = nn.Linear(max_length, emb_dim)
        # self.layernorm = nn.LayerNorm(hid_dim)
        self.pool = nn.AvgPool1d(2, stride=1)
        # self.label_linear = nn.Linear(output_dim, 2)
    ...
    def calculate_attention(self, embedded, conved, encoder_conved, encoder_combined):
        ...
        combined = self.demo(combined)
        combined = torch.tanh(combined)
        encoder_conved = torch.nn.functional.pad(encoder_conved, pad=(1, 0, 0, 0, 0, 0), mode='constant', value=encoder_conved[0][0][0].cpu().detach().item())
        encoder_conved = self.pool(encoder_conved)
    ...
    def forward(self, trg, encoder_conved, encoder_combined):
        ...
        # output_classfical = self.label_linear(output)
        # output_classfical = torch.mean(output_classfical, 1)
        # return output, attention, output_classfical
...
class ConvSeq2Seq(nn.Module):
    ...
    def translate(self, x, sos):
        ...
        for i in range(self.max_length):
            ...
            # pred_value = output[:, :, torch.argmax(output, -1)].max(dim=-2).values

            pred = output.argmax(2)[:,-1].cpu().numpy().tolist()
            # attns.extend(attention)
            preds = np.concatenate((preds, [[i] for i in pred]), -1)
            trg_indexes = np.concatenate((trg_indexes, [[i] for i in pred]), -1)

        return preds.tolist(), attns, True
