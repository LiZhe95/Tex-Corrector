...
class CscTrainingModel(BaseTrainingEngine, ABC):
    ...
    def training_step(self, batch, batch_idx):
        ...
        kl_loss = self.kl_loss(F.log_softmax(outputs[-2] / 8, dim=2), F.softmax(outputs[-1] / 8, dim=2).to(device=outputs[-2].device)) 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for src, tgt, predict, det_predict, det_label in zip(ori_text, cor_y, cor_y_hat, det_y_hat, det_labels):
            ...
            self.valid_cor_acc_labels +=  [1] if operator.eq(_tgt, _predict) else [0]
            self.valid_det_acc_labels += [det_predict[1:len(_src) + 1].equal(det_label[1:len(_src) + 1])]
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self.cfg.logger.info(f'Detection:\n'
                    f'acc: {np.mean(self.valid_det_acc_labels):.4f}')
